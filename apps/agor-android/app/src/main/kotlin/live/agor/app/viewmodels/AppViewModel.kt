...
class AppViewModel(private val container: AppContainer) : ViewModel() {
    ...
    private val _snackbars = MutableSharedFlow<String>(extraBufferCapacity = 16)
    ...
    fun showToast(text: String) {
        _toast.value = text
        _snackbars.tryEmit(text)
    }
    fun consumeToast() { _toast.value = null }

    fun reconnectForForeground() {
        if (container.authService.state.value != AuthState.Authenticated) return
        container.socket.reconnect()
    }
    ...
    fun switchProfile(profile: ServerProfile, onComplete: () -> Unit = {}) {
        viewModelScope.launch {
            container.socket.disconnect()
            container.authService.switchProfile(profile)
            if (container.authService.state.value == AuthState.Authenticated) {
                container.socket.connect()
            }
            onComplete()
