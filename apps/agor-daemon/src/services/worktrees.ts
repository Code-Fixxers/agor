...
export class WorktreesService extends DrizzleService<Worktree, Partial<Worktree>, WorktreeParams> {
  ...
  async create(
    ...
    if (Array.isArray(data)) {
      // ⚡ Bolt Optimization: Batch config loading
      // When creating multiple worktrees, hoist loadConfig() outside the loop.
      // This prevents O(N) disk I/O when applyWorktreeCreateDefaults() is mapped over an array,
      // reducing bulk operation latency significantly.
      const config = await loadConfig();
