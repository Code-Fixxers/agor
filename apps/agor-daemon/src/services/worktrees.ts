...
export class WorktreesService extends DrizzleService<Worktree, Partial<Worktree>, WorktreeParams> {
  ...
  async create(
    ...
    // ⚡ Bolt Optimization: Batch configuration loading.
    // Hoisting loadConfig() outside the map loop prevents O(N) synchronous disk I/O reads.
    // Impact: Significantly reduces latency when creating multiple worktrees concurrently.
    const config = await loadConfig();
