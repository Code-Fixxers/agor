...
export class WorktreesService extends DrizzleService<Worktree, Partial<Worktree>, WorktreeParams> {
  ...
  async create(
    ...
    // ⚡ Bolt Optimization: loadConfig reads and parses from disk on every call.
    // Load it exactly once here to prevent O(N) disk I/O when processing multiple worktrees in an array.
    const config = await loadConfig();
