...
export class WorktreesService extends DrizzleService<Worktree, Partial<Worktree>, WorktreeParams> {
  ...
  async create(
    ...
    // Load config once outside of the loop to prevent O(N) disk I/O when creating many worktrees.
    // Impact: Avoids parsing the config.yaml file from disk for every single item in the array.
    const config = await loadConfig();
