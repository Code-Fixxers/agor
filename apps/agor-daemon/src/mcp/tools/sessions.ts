...
export function registerSessionTools(server: McpServer, ctx: McpContext): void {
  ...
  server.registerTool(
    'agor_sessions_get',
    {
      description: 'Get a session with genealogy, state, and a `url` field.',
      annotations: { readOnlyHint: true },
      inputSchema: z.object({
  ...
  server.registerTool(
    ...
    {
      description:
        'Get the current session plus denormalized worktree/repo/board context. Useful for IDs needed by other tools.',
      annotations: { readOnlyHint: true },
      inputSchema: z.object({}),
    },
    async () => {
  ...
  server.registerTool(
    ...
    {
      description:
        'Create an independent session in a worktree (no parent-child link). MCPs inherit from worktree > user defaults.',
      inputSchema: z.object({
        ...
        mcpServerIds: z.array(z.string()).optional().describe('MCP IDs. Omit to inherit.'),
        modelConfig: modelConfigInputSchema,
      }),
    },
    async (args) => {
