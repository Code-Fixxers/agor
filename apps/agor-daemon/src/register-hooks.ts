...
export function registerHooks(ctx: RegisterHooksContext): void {
  ...
  const injectPerUserOAuthTokens = async (context: HookContext) => {
    ...
    const isArrayResult = Array.isArray(context.result);
    const isPaginatedResult =
      !isArrayResult && context.result?.data && Array.isArray(context.result.data);
    ...
    if (items.length > 0) {
      ...
      const serverIds = items
        .filter((server: MCPServer) => server.auth?.type === 'oauth')
        .map((server: MCPServer) => server.mcp_server_id as import('@agor/core/types').MCPServerID);

      let tokenMap = new Map<string, UserMCPOAuthToken>();
      if (serverIds.length > 0) {
        tokenMap = await userTokenRepo.getTokensForServers(tokenUserId, serverIds);
      }

      const injectTokenOptimized = async (server: MCPServer) => {
        if (server.auth?.type !== 'oauth') {
          return server;
        }

        const mode = server.auth.oauth_mode ?? 'per_user';
        const tUserId = mode === 'per_user' ? tokenUserId : null;
        const key = `${server.mcp_server_id}:${tUserId ?? 'null'}`;
        ...
        try {
          ...
          if (!row) {
            console.log(
              `[MCP OAuth] No token row for user=${tUserId ?? '<shared>'} server=${server.name}`
            );
            return server;
          }
          ...
          if (needsRefresh(row.oauth_token_expires_at) && row.oauth_refresh_token) {
            ...
            try {
              ...
              const fresh = await userTokenRepo.getToken(tUserId, server.mcp_server_id);
      ...
      if (isArrayResult) {
        context.result = await Promise.all(context.result.map(injectTokenOptimized));
      } else if (isPaginatedResult) {
        context.result.data = await Promise.all(context.result.data.map(injectTokenOptimized));
      } else {
        context.result = await injectTokenOptimized(context.result);
      }
    }
