...
export function jsonExtract(db: Database, column: SQL.Aliased | SQL | any, path: string): SQL {
  ...
  if (isSQLiteDatabase(db)) {
    ...
    // IMPORTANT: Explicitly cast parameters to text using ::text so PostgreSQL can resolve the overloaded operator type
    if (parts.length === 1) {
      // Single level: column->>('key'::text)
      return sql`${column} ->> (${parts[0]}::text)`;
    } else {
      // Multiple levels: column->('key1'::text)->('key2'::text)->>('key3'::text)
