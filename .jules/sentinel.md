...
## 2025-06-25 - [SQL Injection Risk via sql.raw in jsonExtract]
**Vulnerability:** A SQL injection vulnerability in `jsonExtract` inside `packages/core/src/db/database-wrapper.ts`. The code used `sql.raw` to interpolate JSON paths manually without parameterization.
**Learning:** Using `sql.raw` to join dynamically generated strings bypasses Drizzle's parameterization, exposing the app to SQL injection if a path is user-controlled.
**Prevention:** For PostgreSQL JSON operators (`->`, `->>`), explicitly cast dynamic parameters to text (e.g., `sql\`${column} ->> (${key}::text)\``) instead of using `sql.raw()`.
