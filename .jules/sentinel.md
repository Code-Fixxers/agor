...
## 2025-05-13 - [SQL Injection Risk via sql.raw in jsonExtract]
**Vulnerability:** A SQL injection vulnerability in `jsonExtract` inside `packages/core/src/db/database-wrapper.ts`. The code used `sql.raw` to interpolate JSON keys into PostgreSQL JSON path operators (`->`, `->>`), bypassing Drizzle's parameterization.
**Learning:** Using `sql.raw` with dynamically generated or unsanitized strings for JSON path keys exposes the application to SQL injection. PostgreSQL overloaded JSON operators require explicit type casting when parameterized.
**Prevention:** Always use Drizzle ORM's parameterized template literals with explicit casting (e.g., `sql\`->>(${key}::text)\``) instead of `sql.raw` to safely construct JSON paths in PostgreSQL.
