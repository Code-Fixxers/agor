...
## 2025-05-23 - [SQL Injection Risk via sql.raw in Postgres JSON Path Extraction]
**Vulnerability:** The `jsonExtract` utility in `packages/core/src/db/database-wrapper.ts` used `sql.raw()` to interpolate dynamic string paths into PostgreSQL JSON extraction operators (`->` and `->>`). This created a SQL injection vector if user-controlled input was passed into the path.
**Learning:** When dynamically chaining multiple JSON path segments in PostgreSQL using Drizzle, `sql.raw()` should be avoided. Drizzle and PostgreSQL support parameterization for JSON operator keys if the parameters are explicitly cast to `text`.
**Prevention:** For PostgreSQL JSON operators (`->`, `->>`), always explicitly cast dynamic parameters to text (e.g., `sql\`${column} ->> (${key}::text)\``) so PostgreSQL can correctly infer the overloaded operator type and avoid syntax errors while preventing SQL injection. When combining segments, use `sql.join` on an array of `sql\`->(${p}::text)\``.
