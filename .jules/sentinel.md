...
**Vulnerability:** A SQL injection vulnerability in `jsonExtract` inside `packages/core/src/db/database-wrapper.ts`. The code used `sql.raw` to interpolate JSON keys into the `->>` and `->` operator syntax without parameterization, enabling an injection vector if the `path` argument was derived from unescaped or malicious input.
**Learning:** Using `sql.raw` directly for string interpolation within Drizzle ORM skips parameterization. Although PostgreSQL's JSON operators are overloaded and typically reject generic parameterized types (causing issues), explicitly casting dynamic inputs (`::text`) enables standard parameterization to function safely without errors.
**Prevention:** Always use parameterized inputs with Drizzle's `sql` template literal for dynamic column lookups. For PostgreSQL JSON operators, use explicit casting like `sql\`->> (${key}::text)\`` instead of manual string concatenation or `sql.raw`.
