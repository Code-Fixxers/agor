...
## $(date +%Y-%m-%d) - [SQL Injection via sql.raw in jsonExtract PostgreSQL query]
**Vulnerability:** A potential SQL injection vulnerability in `jsonExtract` inside `packages/core/src/db/database-wrapper.ts`. The code used `sql.raw` to interpolate user-provided dot-paths directly into a JSON `->>` query in PostgreSQL.
**Learning:** Using `sql.raw` to insert path keys into PostgreSQL JSON query operators makes them vulnerable to SQL injection.
**Prevention:** Use `sql` string templates and cast to `::text` to properly parameterize PostgreSQL JSON operator paths instead of using `sql.raw`.
