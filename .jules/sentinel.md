...
**Vulnerability:** A potential SQL injection vulnerability in `jsonExtract` inside `packages/core/src/db/database-wrapper.ts`. The code used `sql.raw` to construct JSON extraction paths dynamically without parameterization.
**Learning:** Using `sql.raw` to join dynamically generated strings bypasses Drizzle's parameterization, exposing the app to SQL injection if any property key is manipulated.
**Prevention:** Always use Drizzle ORM's built-in parameterization and explicitly cast dynamic parameters to text (e.g., `sql\`->> (${key}::text)\``) to properly resolve the overloaded operator type instead of manual string concatenation or `sql.raw`.
