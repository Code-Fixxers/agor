...
## 2024-06-24 - [CRITICAL] Command Injection via execSync with dynamic input
**Vulnerability:** Command injection vulnerability in `getUidFromUsername`, `getGidFromGroupName`, `getHomedirFromUsername` in `packages/core/src/unix/id-lookups.ts` and `unixUserExists` in `packages/core/src/unix/user-manager.ts`. These functions used `execSync` with string interpolation for `groupName` and `username`.
**Learning:** `execSync` executes commands within a shell (`/bin/sh` on Unix by default). String interpolation allows attackers to append shell metacharacters and execute arbitrary code. Even when quoting is used, quotes can be broken out of.
**Prevention:** Always use `execFileSync` (or `execFile` for async) with arguments passed as an array to bypass shell interpretation entirely. Additionally, place a double-dash `--` before dynamic arguments (e.g., `execFileSync('id', ['-u', '--', username])`) to prevent argument injection where a dynamic input starting with `-` could be misconstrued as a command flag.
