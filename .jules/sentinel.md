...
## 2025-07-22 - [SQL Injection Risk via sql.raw in JSON Extract]
**Vulnerability:** A potential SQL injection vulnerability in `jsonExtract` in `packages/core/src/db/database-wrapper.ts` due to using `sql.raw` to concatenate dynamic JSON paths for PostgreSQL queries.
**Learning:** Using `sql.raw()` bypasses Drizzle's parameterization, exposing the app to SQL injection if any part of the path is untrusted.
**Prevention:** Never use `sql.raw()` to interpolate dynamic variables. To prevent SQL injection when working with PostgreSQL JSON operators (`->`, `->>`), always explicitly cast dynamic parameters to text (e.g., `sql\`->> (${key}::text)\``) to properly resolve the overloaded operator type. Use `sql.join` to safely combine these parameterized template literals when chaining multi-level paths.
