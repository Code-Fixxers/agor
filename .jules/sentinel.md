...
## 2025-05-13 - [Command Injection Risk via execSync]
**Vulnerability:** A potential command injection vulnerability in `packages/core/src/unix/id-lookups.ts` and `packages/core/src/unix/user-manager.ts` where `execSync` was used with string interpolation for dynamic inputs like `username` or `groupName`.
**Learning:** Using `execSync` with dynamic inputs evaluates the string within a shell, allowing shell metacharacters to inject arbitrary commands.
**Prevention:** Always use `execFileSync` or similar functions that accept an argument array, and use `--` to prevent argument injection.
