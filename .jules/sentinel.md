...
## 2025-02-23 - Prevent Command Injection in System Queries
**Vulnerability:** Found `execSync` used with interpolated variables (`username`, `groupName`) inside `packages/core/src/unix/id-lookups.ts`, introducing a command injection vulnerability.
**Learning:** `execSync` interpolates variables inside a shell, which can be easily hijacked via shell metacharacters.
**Prevention:** Always use `execFileSync` (or similar variants) passing parameters in an array. Further, when dynamic arguments are passed, separate them using `--` to prevent argument injection where variable inputs starting with `-` might be confused for command flags.
