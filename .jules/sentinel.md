...
## 2024-05-13 - [Command Injection Risk via Dynamic Input in Exec]
**Vulnerability:** A potential command injection vulnerability in `packages/executor/src/commands/unix.ts` where dynamic input `unixUsername` was directly interpolated into a shell command `id ${unixUsername} > /dev/null 2>&1` via `execAsync`.
**Learning:** Using string interpolation with `exec` or `execAsync` to run shell commands with dynamic inputs bypasses input validation, exposing the system to command injection risks.
**Prevention:** Always use safe, centralized helper commands like `UnixUserCommands.userExists()` that properly format inputs and avoid direct string interpolation in `exec` calls.
