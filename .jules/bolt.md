...
## 2024-05-17 - [Optimizing Feathers Service Lookups]
**Learning:** [When dealing with an N+1 query loop fetching individual items via FeathersJS service methods (`app.service('entity').get(id)`), the service type must be type-cast before calling newly added custom repository methods on the service wrapper to satisfy TypeScript.]
**Action:** [When implementing custom batched fetching methods (like `getMany`) on Feathers service instances, use `as unknown as { customMethod: (...) => ... }` to bypass the strict standard Feathers service typings without introducing unsafe `any` casts.]
