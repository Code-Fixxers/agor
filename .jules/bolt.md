...
## 2024-06-20 - [Fix Disk I/O bottleneck in worktree creation via `applyWorktreeCreateDefaults`]
**Learning:** `loadConfig()` function in `@agor/core/config` reads and parses `~/.agor/config.yaml` from disk synchronously every time it's called. Calling this inside `applyWorktreeCreateDefaults` inside a `Promise.all` loop for bulk operations invokes O(N) disk I/O operations, which drastically impacts performance.
**Action:** Pre-load the config once before iterating over the data array. Then pass the cached config object (or its defaults) to the mapping function.
